...
jest.mock('react-datepicker', () => {
  ...
  return function MockDatePicker({ selected, onChange, customInput, placeholderText }) {
    const handleClick = () => {
      if (onChange) {
...
jest.mock('react-select', () => {
  return function MockSelect({ options, value, onChange, placeholder, classNamePrefix }) {
...
describe('SurplusFormModal', () => {
  ...
  test('does not render modal when isOpen is false', () => {
    const { container } = render(<SurplusFormModal {...defaultProps} isOpen={false} />);
  ...
  test('submits form with valid data', async () => {
    ...
    // Fill location
    await userEvent.type(
      screen.getByPlaceholderText('Start typing to search the location'),
      'Test Location'
    );

    // Fill description
    await userEvent.type(
      screen.getByPlaceholderText('e.g.,Vegetarian lasagna made with tomato sauce, spinach, and ricotta cheese.'),
      'Test description'
    );
  ...
  test('updates form state when input fields change', async () => {
    ...
    await userEvent.type(foodNameInput, 'Banana Bread');

    expect(foodNameInput.value).toBe('Banana Bread');
    ...
    const notesInput = screen.getByPlaceholderText('e.g.,Vegetarian lasagna made with tomato sauce, spinach, and ricotta cheese.');
