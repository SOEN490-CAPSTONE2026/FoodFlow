...
import DonorDashboardHome from '../components/DonorDashboard/DonorDashboardHome';
...
describe('DonorDashboardHome', () => {
  ...
  test('renders metrics and charts with provided data (scoped queries)', () => {
    ...
    const totalListedCard = within(metricsEl)
      .getByText(/total listed items/i)
      .closest('.metric-card');
    expect(totalListedCard).not.toBeNull();
    expect(
      within(totalListedCard).getByText(String(stats.totalListed))
    ).toBeInTheDocument();

    const completedCard = within(metricsEl)
      .getByText(/completed requests/i)
      .closest('.metric-card');
    expect(completedCard).not.toBeNull();
    expect(
      within(completedCard).getByText(String(stats.completed))
    ).toBeInTheDocument();

    const pendingCard = within(metricsEl)
      .getByText(/pending requests/i)
      .closest('.metric-card');
    expect(pendingCard).not.toBeNull();
    expect(
      within(pendingCard).getByText(String(stats.newRequests))
    ).toBeInTheDocument();
    ...
    expect(
      within(rejectedCard).getByText(String(stats.rejected))
    ).toBeInTheDocument();

    // Tiles
    const totalListedTile = screen
      .getByRole('heading', { name: /total listed food/i })
      .closest('.donor-tile');
    expect(totalListedTile).not.toBeNull();
    expect(
      within(totalListedTile).getByText(String(stats.totalListed))
    ).toBeInTheDocument();
    ...
    expect(
      within(completedTile).getByText(String(stats.completed))
    ).toBeInTheDocument();

    const rejectedTile = screen
      .getByRole('heading', { name: /^rejected requests$/i })
      .closest('.donor-tile');
    expect(rejectedTile).not.toBeNull();
    expect(
      within(rejectedTile).getByText(String(stats.rejected))
    ).toBeInTheDocument();

    const allRequestsTile = screen
      .getByRole('heading', { name: /all requests/i })
      .closest('.donor-tile');
    expect(allRequestsTile).not.toBeNull();
    expect(
      within(allRequestsTile).getByText(String(stats.allRequests))
    ).toBeInTheDocument();

    const newRequestsTile = screen
      .getByRole('heading', { name: /new requests/i })
      .closest('.donor-tile');
    expect(newRequestsTile).not.toBeNull();
    expect(
      within(newRequestsTile).getByText(String(stats.newRequests))
    ).toBeInTheDocument();
    ...
    const points = document.querySelectorAll(
      '.css-line-chart .line-points .line-point'
    );
