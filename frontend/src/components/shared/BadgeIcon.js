...
import { Gift, Trophy, Award, Heart, Users, Zap, Star, Medal } from 'lucide-react';
...
const getAchievementIcon = (name, category) => {
  ...
  if (nameLower.includes('first') && nameLower.includes('donation')) return Gift;
  if (nameLower.includes('5') || nameLower.includes('five')) return Trophy;
  if (nameLower.includes('10') || nameLower.includes('ten')) return Award;
  if (nameLower.includes('claim')) return Heart;
  if (nameLower.includes('community') || nameLower.includes('helper')) return Users;
  if (nameLower.includes('quick') || nameLower.includes('fast')) return Zap;
  if (nameLower.includes('milestone')) return Star;
  
  // Fallback to category
  if (category === 'MILESTONE') return Medal;
  if (category === 'ENGAGEMENT') return Zap;
      ...
      {showTooltip && (
        <div className="badge-tooltip">
          <div className="badge-tooltip-header">
            ...
            {unlocked && <span className="badge-unlocked-indicator">✓ Unlocked</span>}
          </div>
          ...
          {!unlocked && (
            <div className="badge-tooltip-criteria">
              ...
              <span className="criteria-value">{achievement.criteriaValue} {achievement.criteriaType.toLowerCase().replace('_', ' ')}</span>
              {progress && (
                <div className="badge-progress">
                  <div className="badge-progress-bar">
                    <div 
                      className="badge-progress-fill" 
                      style={{ width: `${(progress.currentValue / achievement.criteriaValue) * 100}%` }}
                    />
                  </div>
                  <span className="badge-progress-text">{getProgressText()}</span>
                </div>
              )}
            </div>
          )}
