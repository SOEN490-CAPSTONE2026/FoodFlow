...
describe('AdminApprovalBanner', () => {
  it('renders the banner with correct heading', () => {
    ...
    expect(screen.getByText(/account pending admin approval/i)).toBeInTheDocument();
  });
  ...
  it('displays approval timeline information', () => {
    render(<AdminApprovalBanner />);
    expect(screen.getByText(/typically takes up to 48 hours/i)).toBeInTheDocument();
  });

  it('mentions email notification will be sent', () => {
    render(<AdminApprovalBanner />);
    expect(screen.getByText(/you'll receive an email notification once your account is approved/i)).toBeInTheDocument();
  });
  ...
  it('has proper semantic structure', () => {
    ...
    expect(container.querySelector('.admin-approval-banner')).toBeInTheDocument();
    
    // Check for content container
    expect(container.querySelector('.admin-approval-content')).toBeInTheDocument();
