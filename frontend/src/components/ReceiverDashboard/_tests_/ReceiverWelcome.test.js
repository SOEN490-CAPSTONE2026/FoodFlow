...
describe("ReceiverWelcome", () => {
  ...
  test("renders notice tip about pickup hours", () => {
    ...
    expect(
      screen.getByText(/Tip: Set your pickup hours and capacity to get matched faster/i)
    ).toBeInTheDocument();
  });
  ...
  test("renders assistance section with email contact", () => {
    ...
    const emailLink = screen.getByRole("link", { name: /foodflow.group@gmail.com/i });
    expect(emailLink).toBeInTheDocument();
    expect(emailLink).toHaveAttribute("href", "mailto:foodflow.group@gmail.com");
  });

  test("renders FAQ section with link", () => {
    render(<ReceiverWelcome />);
    
    expect(screen.getByText(/New here\? Read common questions/i)).toBeInTheDocument();
