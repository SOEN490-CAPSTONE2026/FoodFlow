...
describe('API service', () => {
  ...
  test('authAPI.resetPassword', async () => {
    ...
    const data = { email: 'user@test.com', code: '123456', newPassword: 'newpass' };
  ...
  test('surplusAPI.completeSurplusPost', async () => {
    ...
    expect(mockPatch).toHaveBeenCalledWith('/surplus/1/complete', { otpCode: '123456' });
    expect(resp).toEqual({ data: { success: true } });
  });
  ...
  test('surplusAPI.search with minimal filters', async () => {
    ...
    expect(mockPost).toHaveBeenCalledWith('/surplus/search', { status: 'AVAILABLE' });
    expect(resp).toEqual({ data: [] });
  });
  ...
  test('surplusAPI.uploadEvidence', async () => {
    mockPost.mockResolvedValue({ data: { url: 'http://example.com/evidence.jpg' } });
    const { surplusAPI } = require('../api');
  ...
  test('recommendationAPI.getBrowseRecommendations handles error', async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith('Error fetching browse recommendations:', expect.any(Error));
    expect(result).toEqual({});
  ...
  test('recommendationAPI.getRecommendationForPost handles error', async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith('Error fetching post recommendation:', expect.any(Error));
  ...
  test('recommendationAPI.getTopRecommendations handles error', async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith('Error fetching top recommendations:', expect.any(Error));
    expect(result).toEqual([]);
    consoleSpy.mockRestore();
  });
  ...
  test('adminDisputeAPI.getAllDisputes with no filters', async () => {
    ...
    expect(mockGet).toHaveBeenCalledWith(expect.stringContaining('/admin/disputes?'));
  ...
  test('adminDisputeAPI.updateDisputeStatus', async () => {
    ...
    const resp = await adminDisputeAPI.updateDisputeStatus(123, 'RESOLVED', 'Fixed');
  ...
  test('adminDonationAPI.getAllDonations with minimal filters', async () => {
    ...
    expect(mockGet).toHaveBeenCalledWith(expect.stringContaining('/admin/donations?'));
    expect(resp).toEqual({ data: { content: [] } });
  });
  ...
  test('adminDonationAPI.getAllDonations with all filters', async () => {
    ...
    expect(mockGet).toHaveBeenCalledWith(expect.stringContaining('status=COMPLETED'));
    expect(mockGet).toHaveBeenCalledWith(expect.stringContaining('donorId=1'));
    expect(mockGet).toHaveBeenCalledWith(expect.stringContaining('receiverId=2'));
    expect(mockGet).toHaveBeenCalledWith(expect.stringContaining('flagged=true'));
    expect(mockGet).toHaveBeenCalledWith(expect.stringContaining('fromDate=2026-01-01'));
    expect(mockGet).toHaveBeenCalledWith(expect.stringContaining('toDate=2026-01-31'));
    expect(mockGet).toHaveBeenCalledWith(expect.stringContaining('search=food'));
  ...
  test('adminDonationAPI.overrideStatus', async () => {
    ...
    const resp = await adminDonationAPI.overrideStatus(123, 'COMPLETED', 'Admin override');
    
    expect(mockPost).toHaveBeenCalledWith('/admin/donations/123/override-status', {
      newStatus: 'COMPLETED',
  ...
  test('adminVerificationAPI.getPendingUsers with no filters', async () => {
    ...
    expect(mockGet).toHaveBeenCalledWith(expect.stringContaining('/admin/pending-users?'));
    expect(resp).toEqual({ data: { content: [] } });
  });
  ...
  test('adminVerificationAPI.getPendingUsers with all filters', async () => {
    ...
    expect(mockGet).toHaveBeenCalledWith(expect.stringContaining('userType=DONOR'));
    expect(mockGet).toHaveBeenCalledWith(expect.stringContaining('search=food+bank'));
    expect(mockGet).toHaveBeenCalledWith(expect.stringContaining('sortBy=date'));
  ...
  test('adminVerificationAPI.rejectUser', async () => {
    ...
    const resp = await adminVerificationAPI.rejectUser(123, 'Invalid documents', 'Please resubmit');
  ...
  test('notificationPreferencesAPI.updatePreferences', async () => {
    ...
    expect(mockPut).toHaveBeenCalledWith('/user/notifications/preferences', data);
    expect(resp).toEqual({ data: { success: true } });
  });
