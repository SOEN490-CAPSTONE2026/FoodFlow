...
class AuthControllerTest {
        ...
        void login_InvalidCredentials_ReturnsBadRequest() throws Exception {
                ...
                // When & Then - GlobalExceptionHandler returns ErrorResponse format
                mockMvc.perform(post("/api/auth/login")
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(objectMapper.writeValueAsString(request)))
                                .andExpect(status().isBadRequest())
        ...
        void login_DonorRole_Success() throws Exception {
                ...
                AuthResponse response = new AuthResponse("jwt-token-donor", "donor@test.com", "DONOR",
                                "Login successful");
        ...
        void login_ReceiverRole_Success() throws Exception {
                ...
                AuthResponse response = new AuthResponse("jwt-token-receiver", "receiver@test.com", "RECEIVER",
                                "Login successful");
        ...
        void login_MissingContentType_ReturnsUnsupportedMediaType() throws Exception {
                ...
                mockMvc.perform(post("/api/auth/login")
                                ...
                                .andExpect(status().isUnsupportedMediaType()); // 415 is correct for missing content
                                                                               // type
        }
