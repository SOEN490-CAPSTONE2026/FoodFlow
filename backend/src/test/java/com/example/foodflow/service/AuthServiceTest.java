...
class AuthServiceTest {
    ...
    void registerDonor_WithDataStorageConsent_SavesConsentFlag() {
        ...
        verify(userRepository)
                .save(argThat(user -> user.getDataStorageConsent() != null && user.getDataStorageConsent() == true));
    }
    ...
    void registerDonor_WithoutDataStorageConsent_SavesFalseFlag() {
        ...
        // Then
        verify(userRepository)
                .save(argThat(user -> user.getDataStorageConsent() != null && user.getDataStorageConsent() == false));
    }
    ...
    void verifyEmail_InvalidToken_ThrowsException() {
        ...
        assertEquals("Invalid verification link. Please check your email or request a new verification link.",
                ex.getMessage());
    ...
    void verifyEmail_AlreadyVerifiedToken_ThrowsException() {
        ...
        assertEquals("This verification link has already been used. Log into your account to proceed.",
                ex.getMessage());
        verify(verificationTokenRepository).findByToken(tokenValue);
        verify(userRepository, never()).save(any(User.class));
    }
    ...
    void resendVerificationEmail_EmailServiceFails_ThrowsException() throws ApiException {
        ...
        when(verificationTokenRepository.save(any(EmailVerificationToken.class)))
                .thenReturn(new EmailVerificationToken());
        willThrow(new ApiException("Email service error")).given(emailService).sendVerificationEmail(anyString(),
                anyString());
    ...
    void forgotPassword_EmailMethod_EmailServiceFails_ThrowsException() throws ApiException {
        ...
        when(userRepository.findByEmail("user@test.com")).thenReturn(Optional.of(user));
        willThrow(new ApiException("Email service error")).given(emailService).sendPasswordResetEmail(anyString(),
                anyString());
    ...
    void resetPassword_UserNotFound_ThrowsException() {
        ...
        // The actual error message is "Invalid reset code" when code doesn't exist in
        // storage
    ...
    void registerReceiver_WithNullOrganizationType_DefaultsToCharity() {
        ...
        verify(organizationRepository).save(argThat(
                org -> org.getOrganizationType() == com.example.foodflow.model.entity.OrganizationType.CHARITY));
    }
    ...
    void registerDonor_SetsDefaultAccountStatus() {
        ...
        // Then
        verify(userRepository).save(argThat(user -> user.getAccountStatus() == AccountStatus.PENDING_VERIFICATION));
        verify(organizationRepository).save(argThat(org -> org.getVerificationStatus() == VerificationStatus.PENDING));
    }
